...
export function useVoiceCall(conversationId: string | null) {
  ...
  const remoteAudio = useRef(new Map<string, { gain: GainNode; stopMonitor: () => void }>());
  ...
  const createPeer = useCallback((userId: string) => {
    ...
    peer.ontrack = async (event) => { if (event.track.kind === 'video') { const stream = event.streams[0] || new MediaStream([event.track]); setRemoteVideoStreams((items) => items.some((item) => item.userId === userId && item.streamId === stream.id) ? items : [...items, { userId, streamId: stream.id, stream }]); event.track.onended = () => setRemoteVideoStreams((items) => items.filter((item) => item.streamId !== stream.id)); return; } if (remoteAudio.current.has(userId)) return; const context = localAudioContext.current; if (!context) { setError('Аудиосистема не запущена. Переподключитесь к звонку.'); return; } const settings = loadAudioSettings(); const setSinkId = (context as AudioContext & { setSinkId?: (id: string) => Promise<void> }).setSinkId; if (settings.outputDeviceId !== 'default' && setSinkId) await setSinkId.call(context, settings.outputDeviceId).catch(() => undefined); const source = context.createMediaStreamSource(new MediaStream([event.track])); const gain = context.createGain(); const analyser = context.createAnalyser(); gain.gain.value = settings.outputVolume / 100; source.connect(analyser); source.connect(gain).connect(context.destination); const stopMonitor = startVoiceActivityMonitor(analyser, (speaking) => setSpeakingUsers((items) => speaking ? { ...items, [userId]: true } : Object.fromEntries(Object.entries(items).filter(([id]) => id !== userId)))); remoteAudio.current.set(userId, { gain, stopMonitor }); event.track.onended = () => { const audio = remoteAudio.current.get(userId); audio?.stopMonitor(); remoteAudio.current.delete(userId); setSpeakingUsers((items) => Object.fromEntries(Object.entries(items).filter(([id]) => id !== userId))); }; if (context.state === 'suspended') await context.resume().catch(() => setError('Браузер заблокировал звук. Нажмите на страницу и переподключитесь к звонку.')); };
  ...
  const leave = useCallback((notify = true) => {
    ...
    peers.current.forEach((peer) => peer.close()); peers.current.clear(); pendingCandidates.current.clear(); remoteAudio.current.forEach(({ stopMonitor }) => stopMonitor()); remoteAudio.current.clear(); localVoiceMonitor.current?.(); localVoiceMonitor.current = null;
  ...
  useEffect(() => realtime.subscribe(async (event: RealtimeEvent) => {
    ...
    if (event.type === 'voice:left') { peers.current.get(event.userId)?.close(); peers.current.delete(event.userId); const audio = remoteAudio.current.get(event.userId); audio?.stopMonitor(); remoteAudio.current.delete(event.userId); setSpeakingUsers((items) => Object.fromEntries(Object.entries(items).filter(([id]) => id !== event.userId))); setParticipants((items) => items.filter((id) => id !== event.userId)); setRemoteVoiceStates((items) => { const next = { ...items }; delete next[event.userId]; return next; }); }
  }), [conversationId, createPeer, connectAudio, leave]);

  useEffect(() => leave, [leave]);
  useEffect(() => { const apply = (event: Event) => { const settings = (event as CustomEvent<AudioSettings>).detail; if (localGain.current) localGain.current.gain.value = settings.inputVolume / 100; remoteAudio.current.forEach(({ gain }) => { gain.gain.value = settings.outputVolume / 100; }); const context = localAudioContext.current; const setSinkId = (context as (AudioContext & { setSinkId?: (id: string) => Promise<void> }) | null)?.setSinkId; if (context && settings.outputDeviceId !== 'default' && setSinkId) void setSinkId.call(context, settings.outputDeviceId).catch(() => undefined); }; window.addEventListener('mova-audio-settings', apply); return () => window.removeEventListener('mova-audio-settings', apply); }, []);
