...
export function useVoiceCall(conversationId: string | null) {
  ...
  const remoteAudio = useRef(new Map<string, { context: AudioContext; gain: GainNode; stopMonitor: () => void }>());
  ...
  const createPeer = useCallback((userId: string) => {
    ...
    peer.ontrack = async (event) => { if (event.track.kind === 'video') { const stream = event.streams[0] || new MediaStream([event.track]); setRemoteVideoStreams((items) => items.some((item) => item.userId === userId && item.streamId === stream.id) ? items : [...items, { userId, streamId: stream.id, stream }]); event.track.onended = () => setRemoteVideoStreams((items) => items.filter((item) => item.streamId !== stream.id)); return; } if (remoteAudio.current.has(userId)) return; const settings = loadAudioSettings(); const context = new AudioContext(); const setSinkId = (context as AudioContext & { setSinkId?: (id: string) => Promise<void> }).setSinkId; if (settings.outputDeviceId !== 'default' && setSinkId) await setSinkId.call(context, settings.outputDeviceId).catch(() => undefined); const source = context.createMediaStreamSource(new MediaStream([event.track])); const gain = context.createGain(); const analyser = context.createAnalyser(); gain.gain.value = settings.outputVolume / 100; source.connect(analyser); source.connect(gain).connect(context.destination); const stopMonitor = startVoiceActivityMonitor(analyser, (speaking) => setSpeakingUsers((items) => speaking ? { ...items, [userId]: true } : Object.fromEntries(Object.entries(items).filter(([id]) => id !== userId)))); remoteAudio.current.set(userId, { context, gain, stopMonitor }); event.track.onended = () => { const audio = remoteAudio.current.get(userId); audio?.stopMonitor(); if (audio?.context.state !== 'closed') void audio?.context.close().catch(() => undefined); remoteAudio.current.delete(userId); setSpeakingUsers((items) => Object.fromEntries(Object.entries(items).filter(([id]) => id !== userId))); }; await context.resume().catch(() => undefined); };
  ...
  const leave = useCallback((notify = true) => {
    ...
    peers.current.forEach((peer) => peer.close()); peers.current.clear(); pendingCandidates.current.clear(); remoteAudio.current.forEach(({ context, stopMonitor }) => { stopMonitor(); if (context.state !== 'closed') void context.close().catch(() => undefined); }); remoteAudio.current.clear(); localVoiceMonitor.current?.(); localVoiceMonitor.current = null;
  ...
  useEffect(() => realtime.subscribe(async (event: RealtimeEvent) => {
    ...
    if (event.type === 'voice:left') { peers.current.get(event.userId)?.close(); peers.current.delete(event.userId); const audio = remoteAudio.current.get(event.userId); audio?.stopMonitor(); if (audio?.context.state !== 'closed') void audio?.context.close().catch(() => undefined); remoteAudio.current.delete(event.userId); setSpeakingUsers((items) => Object.fromEntries(Object.entries(items).filter(([id]) => id !== event.userId))); setParticipants((items) => items.filter((id) => id !== event.userId)); setRemoteVoiceStates((items) => { const next = { ...items }; delete next[event.userId]; return next; }); }
  }), [conversationId, createPeer, connectAudio, leave]);

  useEffect(() => leave, [leave]);
  useEffect(() => { const apply = (event: Event) => { const settings = (event as CustomEvent<AudioSettings>).detail; if (localGain.current) localGain.current.gain.value = settings.inputVolume / 100; remoteAudio.current.forEach(({ context, gain }) => { gain.gain.value = settings.outputVolume / 100; const setSinkId = (context as AudioContext & { setSinkId?: (id: string) => Promise<void> }).setSinkId; if (settings.outputDeviceId !== 'default' && setSinkId) void setSinkId.call(context, settings.outputDeviceId).catch(() => undefined); }); }; window.addEventListener('mova-audio-settings', apply); return () => window.removeEventListener('mova-audio-settings', apply); }, []);
  ...
  const toggleMute = () => { const next = !muted; if (deafenedRef.current && !next) { deafenedRef.current = false; setDeafened(false); remoteAudio.current.forEach(({ gain }) => { gain.gain.value = loadAudioSettings().outputVolume / 100; }); } mutedRef.current = next; localStream.current?.getAudioTracks().forEach((track) => { track.enabled = !next; }); setMuted(next); if (conversationId) realtime.send({ type: 'voice:state', conversationId, muted: next, deafened: deafenedRef.current }); };
  const toggleDeafen = () => { const next = !deafened; deafenedRef.current = next; setDeafened(next); remoteAudio.current.forEach(({ gain }) => { gain.gain.value = next ? 0 : loadAudioSettings().outputVolume / 100; }); if (next) { mutedRef.current = true; setMuted(true); localStream.current?.getAudioTracks().forEach((track) => { track.enabled = false; }); } if (conversationId) realtime.send({ type: 'voice:state', conversationId, muted: mutedRef.current, deafened: next }); };
