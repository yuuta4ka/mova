...
const databasePath = process.env.MOVA_DATABASE_PATH
  ? resolve(process.env.MOVA_DATABASE_PATH)
  : process.env.AMVERA
    ? '/data/db.json'
    : resolve(projectRoot, '.mova-data/db.json');
...
async function loadDatabase() {
  ...
  try { database = JSON.parse(await readFile(databasePath, 'utf8')); }
  catch { database = structuredClone(emptyDatabase); await persist(); }
}
...
function createToken(userId) { const payload = Buffer.from(JSON.stringify({ userId, exp: Date.now() + 1000 * 60 * 60 * 24 * 30 })).toString('base64url'); const signature = createHmac('sha256', secret).update(payload).digest('base64url'); return `${payload}.${signature}`; }
function tokenUser(token) { try { const [payload, signature] = token.split('.'); const expected = createHmac('sha256', secret).update(payload).digest(); if (!timingSafeEqual(expected, Buffer.from(signature, 'base64url'))) return null; const data = JSON.parse(Buffer.from(payload, 'base64url').toString()); if (data.exp < Date.now()) return null; return database.users.find((user) => user.id === data.userId) || null; } catch { return null; } }
...
async function body(request) { let raw = ''; for await (const chunk of request) { raw += chunk; if (raw.length > 12_000_000) throw new Error('Payload too large'); } return raw ? JSON.parse(raw) : {}; }
function auth(request) { return tokenUser(request.headers.authorization?.replace(/^Bearer\s+/i, '') || ''); }
function isMember(userId, conversationId) { return database.memberships.some((membership) => membership.userId === userId && membership.conversationId === conversationId); }
function messageDto(message) {
  return { ...message, sentAt: message.sentAt || message.createdAt, readBy: Array.isArray(message.readBy) ? message.readBy : [], author: publicUser(database.users.find((item) => item.id === message.authorId)) };
}
function conversationDto(conversation, userId) {
  ...
  const members = memberIds.map((memberId) => database.users.find((user) => user.id === memberId)).filter(Boolean).map(publicUser);
  const storedLastMessage = database.messages.filter((message) => message.conversationId === conversation.id).at(-1) || null;
  const lastMessage = storedLastMessage ? { ...storedLastMessage, sentAt: storedLastMessage.sentAt || storedLastMessage.createdAt, readBy: Array.isArray(storedLastMessage.readBy) ? storedLastMessage.readBy : [] } : null;
  return { ...conversation, members, title: conversation.kind === 'direct' ? members.find((member) => member.id !== userId)?.name || 'Сохранённое' : conversation.title, lastMessage };
}
...
async function handleApi(request, response) {
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/register') {
      const data = await body(request); const email = normalizeEmail(data.email); const name = String(data.name || '').trim(); const password = String(data.password || '');
      if (name.length < 2 || !email.includes('@') || password.length < 8) return json(response, 400, { error: 'Укажите имя, корректную почту и пароль от 8 символов' });
      if (database.users.some((user) => user.email === email)) return json(response, 409, { error: 'Пользователь с такой почтой уже существует' });
      const colors = ['#74DCCB', '#9B83F4', '#FF8D72', '#F3C96B', '#68CFA4'];
      const baseHandle = `@${email.split('@')[0].replace(/[^a-z0-9_.]/gi, '').slice(0, 24) || 'user'}`; let handle = baseHandle; let suffix = 1; while (database.users.some((item) => item.handle === handle)) handle = `${baseHandle}${suffix++}`;
      const user = { id: id('usr'), name, email, handle, color: colors[database.users.length % colors.length], presence: 'online', dndUntil: null, bio: '', avatarDataUrl: '', bannerDataUrl: '', activity: null, lastActiveAt: new Date().toISOString(), passwordHash: hashPassword(password), createdAt: new Date().toISOString() };
      database.users.push(user); await persist(); return json(response, 201, { token: createToken(user.id), user: publicUser(user) });
    }
    if (request.method === 'POST' && url.pathname === '/api/login') {
      const data = await body(request); const user = database.users.find((item) => item.email === normalizeEmail(data.email));
    ...
    if (request.method === 'PATCH' && url.pathname === '/api/profile') {
      const data = await body(request); const name = String(data.name || '').trim(); const handle = String(data.handle || '').trim().toLowerCase();
      if (name.length < 2 || name.length > 40) return json(response, 400, { error: 'Имя должно содержать от 2 до 40 символов' });
      if (!/^@[a-z0-9_.]{3,24}$/.test(handle)) return json(response, 400, { error: 'Юзернейм начинается с @ и содержит 3–24 латинских символа' });
      if (database.users.some((item) => item.id !== user.id && item.handle === handle)) return json(response, 409, { error: 'Этот юзернейм уже занят' });
      const avatarDataUrl = String(data.avatarDataUrl || ''); const bannerDataUrl = String(data.bannerDataUrl || '');
      if ((avatarDataUrl && !avatarDataUrl.startsWith('data:image/')) || avatarDataUrl.length > 700_000) return json(response, 400, { error: 'Аватар слишком большой или имеет неверный формат' });
      if ((bannerDataUrl && !bannerDataUrl.startsWith('data:image/')) || bannerDataUrl.length > 1_800_000) return json(response, 400, { error: 'Шапка слишком большая или имеет неверный формат' });
      Object.assign(user, { name, handle, bio: String(data.bio || '').trim().slice(0, 240), avatarDataUrl, bannerDataUrl, activity: data.activity?.name ? { name: String(data.activity.name).trim().slice(0, 80), startedAt: data.activity.startedAt || new Date().toISOString() } : null });
      await persist(); const dto = publicUser(user); broadcastAll({ type: 'profile:update', user: dto }, user.id); return json(response, 200, { user: dto });
    }
    if (request.method === 'POST' && url.pathname === '/api/presence') {
      const data = await body(request); const allowed = ['online', 'idle', 'dnd', 'invisible']; if (!allowed.includes(data.presence)) return json(response, 400, { error: 'Неизвестный статус' });
      user.presence = data.presence; user.dndUntil = data.presence === 'dnd' ? data.dndUntil || 'forever' : null; user.lastActiveAt = new Date().toISOString(); await persist(); const dto = publicUser(user); broadcastAll({ type: 'presence:update', user: dto }, user.id); return json(response, 200, { user: dto });
    }
    if (request.method === 'GET' && url.pathname === '/api/conversations') {
      const ids = database.memberships.filter((item) => item.userId === user.id).map((item) => item.conversationId);
      return json(response, 200, { conversations: database.conversations.filter((item) => ids.includes(item.id)).map((item) => conversationDto(item, user.id)) });
    }
    if (request.method === 'POST' && url.pathname === '/api/conversations') {
      const data = await body(request); const requestedIds = [...new Set((data.memberIds || []).filter((memberId) => database.users.some((item) => item.id === memberId && item.id !== user.id)))];
      if (data.kind === 'direct' && requestedIds.length === 1) {
        const existing = database.conversations.find((conversation) => conversation.kind === 'direct' && database.memberships.filter((item) => item.conversationId === conversation.id).map((item) => item.userId).sort().join() === [user.id, requestedIds[0]].sort().join());
        if (existing) return json(response, 200, { conversation: conversationDto(existing, user.id) });
      }
      const kind = data.kind === 'direct' ? 'direct' : 'group'; const title = kind === 'group' ? String(data.title || '').trim() : '';
      if (kind === 'group' && title.length < 2) return json(response, 400, { error: 'Введите название группы' });
      if (kind === 'group' && requestedIds.length < 1) return json(response, 400, { error: 'Добавьте хотя бы одного участника' });
      const conversation = { id: id('cnv'), kind, title, createdBy: user.id, createdAt: new Date().toISOString() }; database.conversations.push(conversation);
      for (const userId of [user.id, ...requestedIds]) database.memberships.push({ conversationId: conversation.id, userId, joinedAt: new Date().toISOString() });
      await persist(); const dto = conversationDto(conversation, user.id); broadcastToConversation(conversation.id, { type: 'conversation:new', conversationId: conversation.id }); return json(response, 201, { conversation: dto });
    }
    ...
    if (readMatch && request.method === 'POST') {
      const data = await body(request); const messages = database.messages.filter((message) => message.conversationId === readMatch[1]); const throughIndex = messages.findIndex((message) => message.id === data.throughMessageId);
      if (throughIndex < 0) return json(response, 404, { error: 'Сообщение не найдено' });
      const readAt = new Date().toISOString(); const messageIds = [];
      for (const message of messages.slice(0, throughIndex + 1)) {
        ...
        message.readBy.push({ userId: user.id, readAt }); messageIds.push(message.id);
      }
      if (messageIds.length) { await persist(); broadcastToConversation(readMatch[1], { type: 'message:read', conversationId: readMatch[1], userId: user.id, messageIds, readAt }, user.id); }
      return json(response, 200, { conversationId: readMatch[1], userId: user.id, messageIds, readAt });
    }
    ...
    if (messageMatch && request.method === 'GET') return json(response, 200, { messages: database.messages.filter((message) => message.conversationId === messageMatch[1]).slice(-200).map(messageDto) });
    if (messageMatch && request.method === 'POST') {
      const data = await body(request); const content = String(data.content || '').trim(); const rawAttachment = data.attachment; const attachment = rawAttachment && typeof rawAttachment === 'object' ? { name: String(rawAttachment.name || 'Файл').slice(0, 180), type: String(rawAttachment.type || 'application/octet-stream').slice(0, 120), size: Number(rawAttachment.size || 0), dataUrl: String(rawAttachment.dataUrl || '') } : undefined; if ((!content && !attachment) || content.length > 4000) return json(response, 400, { error: 'Сообщение пустое или слишком длинное' }); if (attachment && (attachment.size > 8_000_000 || !attachment.dataUrl.startsWith('data:') || attachment.dataUrl.length > 11_000_000)) return json(response, 400, { error: 'Файл должен быть меньше 8 МБ' });
      const createdAt = new Date().toISOString(); const message = { id: id('msg'), conversationId: messageMatch[1], authorId: user.id, content, ...(attachment ? { attachment } : {}), createdAt, sentAt: createdAt, readBy: [] }; database.messages.push(message); await persist(); const dto = { ...message, author: publicUser(user) }; broadcastToConversation(message.conversationId, { type: 'message:new', message: dto }); return json(response, 201, { message: dto });
    }
...
async function serveFrontend(request, response) {
  ...
  try { contents = await readFile(filePath); }
    ...
    try { contents = await readFile(filePath); }
    catch { return json(response, 503, { error: 'Frontend не собран' }); }
  }
